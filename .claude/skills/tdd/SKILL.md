---
name: tdd
description: "Rust TDD workflow for anki-atlas. Use when writing new features, fixing bugs, or adding tests with Red-Green-Refactor, cargo test, mockall, testcontainers. Not for running existing tests (act-ci) or fixing CI (fix-ci)."
argument-hint: "[crate-name] [feature-description]"
...
- Running existing tests -> use `/act-ci`
- Fixing CI failures -> use `/ci:fix-ci`
- Docs-only or config-only changes

## Relationship to superpowers

**Philosophy** = `superpowers:test-driven-development` (R-G-R discipline, anti-rationalization).
**Execution** = this skill (cargo commands, mockall, testcontainers, project conventions).

Use both together: superpowers sets the mindset, this skill gives you the Rust tools.
...
# Run ignored tests (slow / Docker-required)
...
For local CI simulation, use `/act-ci`. For fixing CI failures, use `/ci:fix-ci`.
...
| Ignoring clippy warnings | Fix them in REFACTOR phase |
